...
$sqlAll = "SELECT + FROM cases WHERE pid = '{$pid}' ORDER BY vdate ASC";
$sqlPatient = "SELECT * FROM PATIENT WHERE pid = '{$pid}'";

$resCase = pg_query($db, $sqlAll);
$resPatient = pg_query($db, $sqlPatient);
...
$pdf->SetTitle('Multiple Report');
$pdf->SetSubject('Multiple Report');
...
for($j = 0; $j < $countCase; $j++) {
	$sqlVitals = "SELECT * FROM cases NATURAL JOIN patient WHERE cid = '{$caseNum[$j]}'";
	$sqlDiagnosis = "SELECT * FROM patient NATURAL JOIN cases INNER JOIN diagnosis ON diagnosis.did = cases.did WHERE cases.cid = '{$caseNum[$j]}'";
	$sqlRN = "SELECT fname, lname, licno FROM employee NATURAL JOIN cases WHERE cid = '{$caseNum[$j]}'";
	$sqlMD = "SELECT * FROM employee NATURAL JOIN diagnosis INNER JOIN cases ON diagnosis.did = cases.did WHERE cases.cid = '{$caseNum[$j]}'";
